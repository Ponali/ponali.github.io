(()=>{
	...
	function getExp(a){
		switch(a){
			...
			case 20:return (Math.floor((Math.sin((x/20)+(t/300))*Math.sin((x/15)+(t/250))*Math.sin((x/10)+(t/200))+1)*127.5)^((Math.floor((y/64)+Math.sin((x/80)+(t/500))/4)%2)*255))*0x10101;
	...
	if(window.rtGac.vhs){x=x+Math.sin(y/40+t/400)*Math.sin(y/20+t/300)*Math.sin(y/30+t/500)*Math.sin(y/10+t/800)*Math.random()*(4+(y/32));};
	...
	if(window.rtGac.proj){a=((cb)=>{function getRGB(val){return [(val&0xff0000)/0x10000,(val&0xff00)/0x100,val&0xff];};function getVal(rgb){return rgb[0]*0x10000+rgb[1]*0x100+rgb[2]};if(!window.rtGac.projRam||typeof(window.rtGac.projRam)!="object"){window.rtGac.projRam=[0,0,0];};let ca=getRGB((cb)&0xffffff);window.rtGac.projRam=window.rtGac.projRam.map((a,i)=>{if(!a){return 1;};return a+((ca[i]-a)/(2+(Math.sin(t/700)*2)+Math.random()*5))});return getVal(window.rtGac.projRam.map((a)=>{return Math.floor(a)}));})(a);}
	if(window.rtGac.brok){a=((cb)=>{function getRGB(val){return [(val&0xff0000)/0x10000,(val&0xff00)/0x100,val&0xff];};function getVal(rgb){return rgb[0]*0x10000+rgb[1]*0x100+rgb[2]};if(!window.rtGac.projRam||typeof(window.rtGac.projRam)!="object"||(x==0)){window.rtGac.projRam=[1,1,1];};let ca=getRGB((cb)&0xffffff);window.rtGac.projRam=window.rtGac.projRam.map((a,i)=>{if(!a){return 0;};return ca[i]-(a*((y/255)**2));});return getVal(window.rtGac.projRam.map((a)=>{return Math.floor(a)}));})(a)}
	if(window.rtGac.vhs){a=((cb)=>{function getRGB(val){return [(val&0xff0000)/0x10000,(val&0xff00)/0x100,val&0xff];};function getVal(rgb){return rgb[0]*0x10000+rgb[1]*0x100+rgb[2]};if(!window.rtGac.projRam||typeof(window.rtGac.projRam)!="object"||(x+y==0)){window.rtGac.projRam=[128,128,128];};let ca=getRGB((cb)&0xffffff);window.rtGac.projRam=window.rtGac.projRam.map((a,i)=>{if(!a){return 0;};return (ca[i]*0.3)+(a*0.7)+(Math.random()-0.5)*(((y/16)+32)*(1+y/(256/2)));});return getVal(window.rtGac.projRam.map((a)=>{return Math.max(Math.min(Math.floor(a),255),0)}));})(a)}
	return a;
})()